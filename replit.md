...
- **Authentication System:** Login/Register, persistent auth state with AsyncStorage, JWT tokens (mocked for demo, real implementation ready).
...
- **File-Based Routing:** Utilizes Expo Router with `(auth)`, `(tabs)`, `(admin-auth)`, and `(admin-tabs)` groups for layout organization, including dynamic routes like `/campaign/[id]`.
